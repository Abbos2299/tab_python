    ...
    parameters = {
        ...
        "messages": [
            ...
            Fetch following data and print in JSON format:
                    1. "Load number" starting with FB or EL;
                    2. "Broker email" address;
...
def extract_info_from_text(text):
    ...
    # Assign the matched values to the respective variables
    LoadNumber = matches[1]  # Second match
    BrokerEmail = matches[3]  # Fourth match
    Rate = matches[5]  # Sixth match
...
def calculate_total_distance(addresses, api_key):
    ...
    total_distance_miles = total_distance / 1609

    return total_distance_miles

def save_data_to_firestore(user_uid, file_name, LoadNumber, BrokerEmail, Rate, rounded_total_distance_miles, formatted_addresses, processed_date_times):
...
load_number = info["Load number"]
broker_email = info["Broker email"]
load_pay = info["Load Pay Amount"]
# Step 9: 
